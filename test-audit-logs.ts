...
async function testAuditLogs() {
  try {
    ...
    const listRequest: CallToolRequest = {
      method: 'tools/call',
      params: {
    ...
    const listResult = await handleListAuditLogs(listRequest, client);
    ...
    if (logIdMatch) {
      ...
      const getResult = await handleGetAuditLog(getRequest, client);
